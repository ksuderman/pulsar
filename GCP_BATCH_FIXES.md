...
### 3. Working directory output collection
...
### 4. ~~Fix input file staging for lightweight parameter tools~~

**Note:** The proper fix is to send parameter tool jobs to the `local` job runner. Since these tools have no command line there is no way for Batch to run them.
...
### 7. Parameterizable boot disk size

**Commit:** `0e0a995`
**Files:** `pulsar/client/container_job_config.py`

**Problem:** Custom VM images (fix 6) may be larger than the default 30 GB GCP Batch boot disk. When the image exceeds the boot disk size, the VM fails to provision.
...
### 8. CVMFS volume mounts on GCP Batch VMs
...
### 9. Local SSD count validation for N2/N2D machines
...
### 10. Dynamic VM sizing from cores/mem
...
## Job Naming and Lifecycle

### 12. Unique job names with timestamps
...
### 13. Configurable job_id_prefix for GCP Batch job names
...
### 14. Optional GCP Batch job deletion
...
- Fix 7: Parameterizable boot disk size (`0e0a995`)
- Companion Galaxy PR for `PASSTHROUGH_PARAMS` in `PulsarGcpBatchJobRunner` (`11ca3ff006`)

### PR 3: Resource management and job lifecycle
- Fix 9: SSD count validation (`efa5eaa`)
- Fix 10: Dynamic VM sizing (`b194758`)
